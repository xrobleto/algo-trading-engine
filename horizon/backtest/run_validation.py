...
FULL = ("2008-01-02", "2026-05-15")
IS = ("2008-01-02", "2017-12-29")
OOS = ("2018-01-02", "2026-05-15")
EQ = 100_000.0
...
def _build(sid: str, **kw):
    return {"PULSE": PulseStrategy, "ROTATION": RotationStrategy,
            "REVERT": RevertStrategy, "DRIFT": DriftStrategy}[sid](**kw)
...
def _a6(sid: str, dataset) -> tuple:
    ...
    variants = []
    if sid == "PULSE":
        for tv in (0.225, 0.375):
            variants.append(("target_vol=%.3f" % tv, {"target_vol": tv}))
        for vd in (15, 25):
            variants.append(("vol_days=%d" % vd, {"vol_days": vd}))
    elif sid == "ROTATION":
        variants.append(("lookbacks x0.75", {"lookbacks": (47, 95, 189)}))
        variants.append(("lookbacks x1.25", {"lookbacks": (79, 158, 315)}))
...
def main() -> None:
    ...
    strategies = {"PULSE": PulseStrategy(), "ROTATION": RotationStrategy(),
                  "REVERT": RevertStrategy(), "DRIFT": DriftStrategy()}
...
def cfg_docs_path():
    ...
    return PACKAGE_DIR / "docs" / "VALIDATION.md"

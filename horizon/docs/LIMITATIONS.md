...
9. **The live order layer is v1.** It now has email alerting (failures,
   ...
   shadowed keys fail fast instead of silently dropping to dry-run),
   pending-order-aware diffing (a queued batch is never re-submitted on the
   next cycle), an emergency `--flatten` command, orphaned-position recovery
   in the reconciler, and Railway deployment config (Dockerfile +
   railway.json). What remains v1: partial-fill handling is
